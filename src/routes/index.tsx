...
/* ------------------------------------------------------------------ *
 * ✏️  EDIT ME — surprise notes for Himani
 *
 * Add an entry for each day-count you want a pink heart note to appear.
 * `day` = the number of days shown on Himani's card.
 * `note` = the message inside the enlarged post-it.
 * ------------------------------------------------------------------ */
const HIMANI_NOTES: { day: number; note: string }[] = [
  ...
  { day: 10, note: "i love infinity billion trillion infinity times ☹️ i pinky promise i'll be better" },
];
/* ------------------------------------------------------------------ */
...
function Index() {
  ...
  const himaniNote =
    now !== null && !himani.done
      ? HIMANI_NOTES.find((n) => n.day === himani.days)?.note
      : undefined;
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 sm:items-start">
          ...
          <div className="relative">
            <CountdownCard
              ...
              featured
            />
            {himaniNote && <PostIt note={himaniNote} />}
          </div>
        </div>
